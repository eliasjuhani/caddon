...
(function() {
  ...
  window.postMessage({ type: 'COLLECT_STORE_READY' }, '*');
  console.log('C@S Inject: Script ready, config can be loaded');
  ...
  function sendToContentScript(data) {
    try {
      window.postMessage({ type: 'COLLECT_STORE_DATA', data: data }, '*');
    } catch (error) {
  ...
  function clickRefreshButton() {
    try {
      ...
      const knownIds = ['__jsview1--BtnRefresh', '__jsview2--BtnRefresh', '__jsview3--BtnRefresh', '__jsview0--BtnRefresh'];
