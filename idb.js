...
const MediaDB = (function() {
  ...
  function openDB() {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onerror = (event) => {
        console.error('MediaDB: Failed to open database:', event.target.error);
  ...
  async function saveMedia(key, blob, type, fileName) {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        console.log(`MediaDB: Saved ${type} media as '${key}' (${formatBytes(blob.size)})`);
        resolve();
      };
      request.onerror = (event) => {
        console.error(`MediaDB: Failed to save ${key}:`, event.target.error);
        reject(event.target.error);
      };
    });
  }
  async function getMedia(key) {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onerror = (event) => {
        console.error(`MediaDB: Failed to get ${key}:`, event.target.error);
        reject(event.target.error);
      };
    });
  }
  ...
  async function deleteMedia(key) {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        console.log(`MediaDB: Deleted ${key}`);
        resolve();
      };
      request.onerror = (event) => {
        console.error(`MediaDB: Failed to delete ${key}:`, event.target.error);
        reject(event.target.error);
      };
    });
  }
